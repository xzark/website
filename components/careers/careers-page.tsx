...
import { InternationalPhoneInput } from "@/components/careers/international-phone-input"
import { Button } from "@/components/ui/button"
import { Checkbox } from "@/components/ui/checkbox"
import { Input } from "@/components/ui/input"
import { Textarea } from "@/components/ui/textarea"
import { careerTracks, culturePrinciples, jobOpenings, type JobOpening } from "@/lib/careers"
import { getCareersApiError, submitApplication, type CareersApplicationPayload } from "@/services/careersApi"
import { validateApplication, validatePdf, validatePdfMetadata, type FormIssue } from "@/lib/validation/forms"
...
function ApplicationModal({ job, onClose }: { job: JobOpening; onClose: () => void }) {
  ...
  if (step === "confirmation") return <div className="fixed inset-0 z-[70] grid place-items-center bg-background/95 px-4"><div className="w-full max-w-md border border-border/80 bg-card p-8 text-center"><div className="mx-auto flex size-12 items-center justify-center border border-primary/40 bg-primary/10"><Check className="size-5 text-primary" /></div><p className="mt-6 font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Application submitted</p><h2 className="mt-3 text-2xl font-light">Obrigado por se candidatar.</h2><p className="mt-4 text-sm leading-relaxed text-muted-foreground">Sua candidatura para <span className="text-foreground">{job.title}</span> foi enviada para a API externa da xZark.</p><Button onClick={onClose} variant="outline" className="mt-7 rounded-md">Fechar</Button></div></div>
  async function handleSubmit() {
    ...
    try {
      const payload: CareersApplicationPayload = {
        jobId: job.slug,
        candidate: { name: values.name ?? "", email: values.email ?? "", phone: phoneE164, location: values.location ?? "" },
      ...
      await submitApplication(payload, file)
      setStep("confirmation")
    } catch (error) {
      setApiError(getCareersApiError(error))
    } finally {
      setIsSubmitting(false)
    }
  }

  if (step === "review") return <ReviewApplication job={job} values={values} fileName={fileName} apiError={apiError} isSubmitting={isSubmitting} onEdit={() => setStep("form")} onSubmit={handleSubmit} />

  return <div className="fixed inset-0 z-[70] overflow-y-auto bg-background/95 px-4 py-8 backdrop-blur-md"><div className="mx-auto max-w-2xl border border-border/80 bg-card p-6 md:p-10"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Apply now</p><h2 className="mt-3 text-2xl font-light">{job.title}</h2></div><button type="button" onClick={onClose} aria-label="Fechar candidatura" className="text-muted-foreground hover:text-foreground"><X className="size-5" /></button></div><form onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); const nextValues = Object.fromEntries(Array.from(form.entries()).map(([key, value]) => [key, String(value)])); const nextIssues = validateApplication({ name: nextValues.name ?? "", email: nextValues.email ?? "", phone: nextValues.phone ?? "", location: nextValues.location ?? "", linkedin: nextValues.linkedin ?? "", github: nextValues.github ?? "", portfolio: nextValues.portfolio ?? "", experience: nextValues.experience ?? "", education: nextValues.education ?? "", availability: nextValues.availability ?? "", message: nextValues.message ?? "", consent }); if (phoneDisplay && !phoneValid) nextIssues.push({ field: "phone", message: "Please enter a valid phone number." }); setValues({ ...nextValues, phone: phoneE164 }); setIssues(nextIssues); if (!file || fileIssue || nextIssues.length > 0) return; setApiError(""); setStep("review") }} className="mt-8 grid gap-5 md:grid-cols-2"><Field label="Nome" required><Input required name="name" defaultValue={values.name} placeholder="Seu nome" /></Field><Field label="E-mail" required><Input required type="email" name="email" defaultValue={values.email} placeholder="voce@exemplo.com" /></Field><Field label="Telefone"><InternationalPhoneInput country={phoneCountry} value={phoneDisplay} onCountryChange={(nextCountry) => { setPhoneCountry(nextCountry); setPhoneDisplay(""); setPhoneE164(""); setPhoneValid(false) }} onChange={(displayValue, e164Value, isValid) => { setPhoneDisplay(displayValue); setPhoneE164(e164Value); setPhoneValid(isValid) }} /></Field><Field label="Localização" required><Input required name="location" defaultValue={values.location} placeholder="Cidade, país" /></Field><Field label="LinkedIn"><Input name="linkedin" type="url" defaultValue={values.linkedin} placeholder="https://linkedin.com/in/..." /></Field><Field label="GitHub"><Input name="github" type="url" defaultValue={values.github} placeholder="https://github.com/..." /></Field><Field label="Portfolio"><Input name="portfolio" type="url" defaultValue={values.portfolio} placeholder="https://..." /></Field><Field label="Experiência" required><Textarea required name="experience" defaultValue={values.experience} placeholder="Resumo da sua experiência" /></Field><Field label="Formação"><Textarea name="education" defaultValue={values.education} placeholder="Formação acadêmica ou equivalente" /></Field><Field label="Disponibilidade" required><Input required name="availability" defaultValue={values.availability} placeholder="Quando você poderia começar?" /></Field><Field label="Currículo em PDF" required><label className="flex h-9 cursor-pointer items-center gap-2 rounded-md border border-input px-3 text-sm text-muted-foreground hover:border-primary/60"><Upload className="size-3.5" />{fileName || "Selecionar arquivo"}<input required type="file" accept=".pdf,application/pdf" className="sr-only" onChange={async (event) => {
          const nextFile = event.target.files?.[0] ?? null
          const metadataIssue = validatePdfMetadata(nextFile)
          setFileIssue(metadataIssue ?? "Selecting…")
          setFile(nextFile)
          setFileName(nextFile?.name ?? "")
          const issue = await validatePdf(nextFile)
          setFileIssue(issue ?? "")
          if (issue) setFile(null)
        }} /></label>{fileIssue && <span role="alert" className="mt-2 block text-xs text-destructive">{fileIssue}</span>}</Field><Field label="Apresentação / mensagem" className="md:col-span-2"><Textarea name="message" defaultValue={values.message} placeholder="Conte brevemente sobre seu trabalho e o que gostaria de construir." className="min-h-28" /></Field><label className="flex gap-3 md:col-span-2"><Checkbox checked={consent} onCheckedChange={(checked) => setConsent(checked === true)} /><span className="text-xs leading-relaxed text-muted-foreground">Autorizo o tratamento dos dados desta candidatura exclusivamente para fins de recrutamento e seleção.</span></label>{issues.length > 0 && <p role="alert" className="text-sm text-destructive md:col-span-2">{issues[0].message}</p>}<Button type="submit" disabled={!consent || !fileName || Boolean(fileIssue)} className="rounded-md bg-primary text-primary-foreground hover:bg-primary/90 md:col-span-2">Enviar candidatura <ArrowUpRight className="ml-2 size-4" /></Button></form><div className="mt-6 flex items-start gap-2 border-t border-border/60 pt-5 text-[11px] leading-relaxed text-muted-foreground"><FileText className="mt-0.5 size-3.5 shrink-0" />PDF only · Maximum 10 MB. O envio usa a API externa configurada pela xZark quando disponível.</div></div></div>
}

function ReviewApplication({ job, values, fileName, apiError, isSubmitting, onEdit, onSubmit }: { job: JobOpening; values: Record<string, string>; fileName: string; apiError: string; isSubmitting: boolean; onEdit: () => void; onSubmit: () => void }) {
  const rows = [["Nome", values.name], ["E-mail", values.email], ["Telefone", values.phone], ["Localização", values.location], ["LinkedIn", values.linkedin], ["GitHub", values.github], ["Portfolio", values.portfolio], ["Experiência", values.experience], ["Formação", values.education], ["Cover Letter", values.message]]
  return <div className="fixed inset-0 z-[75] overflow-y-auto bg-background/95 px-4 py-8 backdrop-blur-md"><div className="mx-auto max-w-2xl border border-border/80 bg-card p-6 md:p-10"><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-primary">Review Application</p><h2 className="mt-3 text-3xl font-light">Revise seus dados</h2><p className="mt-3 text-sm leading-relaxed text-muted-foreground">Confira as informações antes de enviar sua candidatura para {job.title}.</p><div className="mt-8 divide-y divide-border/60 border-y border-border/60">{rows.map(([label, value]) => <div key={label} className="grid gap-1 py-4 sm:grid-cols-[150px_1fr]"><span className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">{label}</span><span className="whitespace-pre-wrap break-words text-sm text-foreground">{value || "—"}</span></div>)}<div className="grid gap-1 py-4 sm:grid-cols-[150px_1fr]"><span className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted-foreground">CV</span><span className="text-sm text-foreground">{fileName}</span></div></div>{apiError && <p role="alert" className="mt-5 text-sm text-destructive">{apiError}</p>}<div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={onEdit} disabled={isSubmitting} className="rounded-md">Edit</Button><Button type="button" onClick={onSubmit} disabled={isSubmitting} className="rounded-md bg-primary text-primary-foreground hover:bg-primary/90">{isSubmitting ? "Submitting application..." : "Submit Application"}</Button></div></div></div>
}
